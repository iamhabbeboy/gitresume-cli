...
import type { WorkExperience as type } from "./type";

interface Props {
  work: type;
  index: number;
  setModalStatus: React.Dispatch<SetStateAction<number | null>>;
}
const WorkExperience: React.FC<Props> = ({ work, index, setModalStatus }) => {
  const handleEditWorkExperience = (index: number) => {
    setModalStatus(index);
  };
  return (
    <div key={index} className={`${index !== 1 ? "my-5" : ""}`}>
      <div className="flex">
        <h2 className="text-xl">{work.role}</h2>
        <span
          className="ml-2"
